...
const employeeSchema = new mongoose.Schema({
  ...
  workingDays: { type: Number },                       // Working Days (can be 5, 5.5, 20.5, etc.)
  monthPayment: { type: Number, min: 0 },              // Monthly Payment with 2 decimals
  OTHrs: { type: Number },                             // OT Hours (can be hours and seconds)
  OTRate: { type: Number, min: 0 },                    // OT Rate like 20.34, 300.00, etc.
  OTEarning: { type: Number, min: 0 },                 // OT Earnings like 300045.34, etc.
  incomeAllowance: { type: Number, min: 0 },           // Income Allowance like 200.67, etc.
  doubleShiftDays: { type: Number, min: 0 },           // Double Shift Days
  doubleShiftEarning: { type: Number, min: 0 },        // Double Shift Earning like 231432.45, etc.
  attendance25: { type: Number, min: 0 },              // 25 Attendance Days
  grossSalary: { type: Number, min: 0 },               // Gross Salary like 2324354.34, etc.
  EPF: { type: Number, min: 0 },                       // EPF like 2342.23, etc.
  totalDeduction: { type: Number, min: 0 },            // Total Deduction like 324234.34, etc.
  netSalary: { type: Number, min: 0 }                  // Net Salary like 14543543.78, etc.
});
